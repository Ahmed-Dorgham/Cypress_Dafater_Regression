...
it('TC01_create New Purchase Receipt And Submit',function () {
...
cy.get('input[data-fieldname="item_code"][placeholder="رمز الصنف"]', { timeout: 180000 }).eq(1).scrollIntoView({ force: true }).should('be.visible');
...
cy.log('get status of purchase receipt Before Creating Related Purchase Invoice');
cy.get('.indicator-pill.no-indicator-dot.whitespace-nowrap.orange',{timeout:180000}).eq(0).should('exist').should('be.visible')
.invoke('text').as('purchaseReceiptStatusBeforeCreatingRelatedPurchaseInvoice');

cy.get('@purchaseReceiptStatusBeforeCreatingRelatedPurchaseInvoice').then(value => {
  cy.log(`status of  purchase Receipt  Before Creating Related Purchase Invoice is : ${value}`)});

cy.log('get status of purchase Receipt ');
cy.get('.label.label-success',{timeout:180000}).should('exist').should('be.visible')
.invoke('text').as('purchaseReceiptStatus');

cy.get('@purchaseReceiptStatus').then(value => {
  cy.log(`purchase Receipt status is  : ${value}`)});

  cy.log(' verify that the status of created purchase Receipt is submitted معتمد ')
  cy.get('@purchaseReceiptStatus').then(name => {
  const cleanName = name.trim();

...
describe('Write Off Sales Invoices Suite ', function () {
...
const paidAmount = '1000';
  ...
  it('TC01_create New Sales Invoice And Save After Apply Complete Write Off ',function () {
...
// cy.get('div[data-fieldname="grand_total"]',{timeout:180000}).eq(0).find('span[dir="rtl"]').should('not.contain.text', '0.000');
...
// cy.get('@salesInvoiceSGrandTotalAmount').then(value => {
//   cy.log(`sales Invoice  Total outstanding Amount before applying complete writeOff is  : ${value}`)});


// cy.log('open payments section');
// cy.get('#sales-invoice-payments_tab-tab',{timeout:180000}).should('exist').click();

// cy.log('enter write off amount');
// cy.get('[data-fieldname="write_off_section"]',{timeout:180000}).should('exist').click();
// cy.get('[id="write_off_amount"]',{timeout:180000}).should('exist').click();
// cy.get('[id="write_off_amount"]',{timeout:180000}).should('exist').clear();


// cy.get('@salesInvoiceSGrandTotalAmount').then(value => {
// cy.get('[id="write_off_amount"]',{timeout:180000}).should('exist').type(`${value}`);
// });

// cy.get('[id="write_off_account"]',{timeout:180000}).should('exist').click();
...
// cy.log('get total outstanding amount value  after apply write off ');
// cy.get('div[data-fieldname="outstanding_amount"]',{timeout:180000}).eq(0).find('span[dir="rtl"]').scrollIntoView().should('exist')
// .invoke('text').as('salesInvoiceTotalOutstandingAmountAfterApplyCompleteWriteOff');

// cy.get('@salesInvoiceTotalOutstandingAmountAfterApplyCompleteWriteOff').then(value => {
//   cy.log(`total outstanding amount value  after apply complete write off is : ${value}`)});

// cy.log("verify that outstanding amount before applying complete writeOff not equal to outstanding amount after applying complete writeOff");
// cy.get('@salesInvoiceSGrandTotalAmount').then((outStandingBeforeWriteOff) => {
//   cy.get('@salesInvoiceTotalOutstandingAmountAfterApplyCompleteWriteOff').then((outStandingAfterWriteOff) => {

//     const outStandingBefore = outStandingBeforeWriteOff.trim()
//     const outStandingAfter = outStandingAfterWriteOff.trim()

//     expect(outStandingBefore).not.eq(outStandingAfter)
//   })
// });
...
/*********************************************************************************************************************************************** */
it('TC02_create New Sales Invoice When Apply WriteOff And Pay Advance Amount With Value Equal To Grand Total Amount ',function () {
...
cy.log('click on receipt voucher  option ');
cy.get('#sidebar-selling-receipt-vouchers',{timeout:180000}).should('exist').as('receiptVouchersTab');
cy.get('@receiptVouchersTab').click({force:true});
// cy.contains("مسودة",{timeout:300000}).should('be.visible');


cy.log('click on new receipt voucher btn  ');
cy.contains('إضافة سندات الدفع',{timeout:180000}).should('be.visible').click();
cy.contains('سندات الدفع جديد',{timeout:180000});

cy.log('select customer ');
cy.get('[data-target="party_type"][placeholder=" "]',{timeout:180000}).eq(0).should('exist').scrollIntoView().click();
cy.get('[data-target="party_type"][placeholder=" "]',{timeout:180000}).eq(0).should('exist').click();
cy.get('ul[role="listbox"]:not([hidden])', { timeout: 180000 }).should('exist').find('div').first().find('p').first({timeout: 180000 }).should('exist').should('be.visible').click();
  

cy.log('select account ');
cy.get('[id="paid_to"][placeholder=" "]',{timeout:180000}).eq(0).should('be.visible').click();
cy.get('[id="paid_to"][placeholder=" "]',{timeout:180000}).eq(0).should('be.visible').click();
cy.get('ul[role="listbox"]:not([hidden])', { timeout: 180000 }).should('exist').find('div').first().find('p').first({timeout: 180000 }).should('exist').should('be.visible').click({force:true});
  
cy.log(' enter paid amount  ');
cy.get('[id="paid_amount"][placeholder=" "]',{timeout:180000}).eq(0).should('exist').scrollIntoView().should('be.visible').click({force:true});
cy.get('[id="paid_amount"][placeholder=" "]',{timeout:180000}).eq(0).should('exist').type(paidAmount);
...
cy.log('open payments section');
cy.get('#sales-invoice-payments_tab-tab',{timeout:180000}).should('exist').click({force:true});

cy.log('pay with advanced amount');
cy.get('[data-fieldname="advances_section"]',{timeout:180000}).should('exist').click();
cy.get('[id="get_advances"]',{timeout:180000}).should('exist').click();


cy.get('[data-name*="new-sales-invoice-advance"]',{timeout:180000}).eq(0).should('exist').find('div[data-fieldname*="allocated_amount"]').find('span').click();

cy.get('[data-name*="new-sales-invoice-advance"]',{timeout:180000}).eq(0).should('exist').find('div[data-fieldname*="allocated_amount"]').find('input').click().clear()
.type(75).clear().type(75);

cy.log('enter write off amount');
cy.get('[data-fieldname="write_off_section"]',{timeout:180000}).should('exist').scrollIntoView({force:true}).should('be.visible').click();
cy.get('[data-fieldname="write_off_section"]',{timeout:180000}).should('exist').click();
cy.get('[id="write_off_amount"]',{timeout:180000}).should('exist').click();
cy.get('[id="write_off_amount"]',{timeout:180000}).should('exist').clear();
...
cy.get('@salesInvoiceSGrandTotalAmount').then(value => {
cy.get('[id="write_off_amount"]',{timeout:180000}).should('exist').type(75);
});


cy.get('[id="write_off_account"]',{timeout:180000}).should('exist').click();

cy.get('[class*="msgprint"]',{timeout:30000}).should('exist')
.invoke('text').as('writeOffValidationMsg');


cy.get('@writeOffValidationMsg').then(value => {
  cy.log(`validation msg is  : ${value}`)});
...
it.skip('TC08_create New Sales Invoice And Check In General Ledger Report',function () {
...
cy.get('@valueAtDefaultIncomeAccountGL').then((incomeGLValue) => {
  cy.get('@salesInvoiceSTotalAmount').then((invoiceTotal) => {
